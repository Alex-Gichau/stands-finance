...
async function startServer() {
  ...
  app.post("/api/config/migrate-data", async (req, res) => {
    ...
    try {
      ...
      return res.json({
        success: true,
        message: "Supabase is configured as the primary data source. Firebase is no longer required for this workflow.",
        stats: {
          supabase: { fetched: 0, migrated: 0, errors: 0 }
        }
      });
    } catch (err: any) {
      return res.status(500).json({
        success: false,
        error: "Supabase connection verification failed: " + (err.message || String(err))
      });
    }
  });
