...
function AppContent() {
  ...
  const runDiagnostics = async () => {
    ...
    try {
      const res = await fetch("/api/config/troubleshoot", { method: "POST" });
      if (res.ok) {
        const data = await res.json();
        setDiagnosticReport(data);
      } else {
        setDiagnosticError(`Diagnostic API returned status: ${res.status}`);
      }
    } catch (err: any) {
      setDiagnosticError(err.message || "Failed to contact diagnostic server API.");
    } finally {
      ...
      <div className="h-[100vh] w-[100vw] bg-slate-950 flex items-center justify-center sm:p-6 relative overflow-hidden">
        ...
        >
          ...
          <div className="space-y-6">
            ...
            {/* Connection Diagnostics Sub-panel */}
            <div className="mt-4 pt-4 border-t border-slate-800/60 text-center">
              <button
                ...
                className="text-[9px] font-black text-slate-500 hover:text-primary uppercase tracking-widest transition-all inline-flex items-center gap-1 cursor-pointer"
              >
                <RefreshCw size={10} className={isDiagnosing ? "animate-spin text-primary" : ""} />
                {showDiagnostics ? "Hide Connection Diagnostics" : "Database & Connection Diagnostics"}
              </button>
              ...
              <AnimatePresence>
                {showDiagnostics && (
                  <motion.div
                    ...
                    className="overflow-hidden text-left mt-3 bg-slate-950/60 rounded-xl p-4 border border-slate-800/80 space-y-3"
                  >
                    <h4 className="text-[10px] font-black text-white uppercase tracking-wider flex items-center justify-between">
                      <span>Connection Health Check</span>
                      {isDiagnosing && <Loader2 className="animate-spin text-primary" size={12} />}
                    </h4>

                    {isDiagnosing && (
                      <p className="text-[10px] text-slate-400 font-semibold flex items-center gap-2">
                        Auditing network route & secrets...
                      </p>
                    )}

                    {diagnosticError && (
                      <div className="p-2.5 bg-rose-950/30 border border-rose-900/40 rounded-lg text-[10px] text-rose-400 font-bold leading-normal">
                        {diagnosticError}
                      </div>
                    )}
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div className="p-1.5 bg-slate-900/80 rounded-lg border border-slate-800 text-center">
                            <p className="text-[7px] font-bold text-slate-500 uppercase tracking-wider">REST URL</p>
                            <span className={`text-[9px] font-black ${diagnosticReport.env?.SUPABASE_URL ? "text-emerald-500" : "text-rose-500"}`}>
                              {diagnosticReport.env?.SUPABASE_URL ? "SET" : "MISSING"}
                            </span>
                          </div>
                          <div className="p-1.5 bg-slate-900/80 rounded-lg border border-slate-800 text-center">
                            <p className="text-[7px] font-bold text-slate-500 uppercase tracking-wider">REST KEY</p>
                            <span className={`text-[9px] font-black ${diagnosticReport.env?.SUPABASE_ANON_KEY ? "text-emerald-500" : "text-rose-500"}`}>
                              {diagnosticReport.env?.SUPABASE_ANON_KEY ? "SET" : "MISSING"}
                            </span>
                          </div>
                          <div className="p-1.5 bg-slate-900/80 rounded-lg border border-slate-800 text-center">
                            <p className="text-[7px] font-bold text-slate-500 uppercase tracking-wider">DB URL</p>
                            <span className={`text-[9px] font-black ${diagnosticReport.env?.DATABASE_URL ? "text-emerald-500" : "text-rose-500"}`}>
                              {diagnosticReport.env?.DATABASE_URL ? "SET" : "MISSING"}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5 text-[10px] font-bold">
                          <div className="flex items-center justify-between py-1 border-b border-slate-900">
                            <span className="text-slate-400">Direct SQL (pg-driver):</span>
                            <span className={diagnosticReport.postgres?.connected ? "text-emerald-500" : "text-rose-500"}>
                              {diagnosticReport.postgres?.connected ? "🟢 Reachable" : "❌ Refused/Mismatch"}
                            </span>
                          </div>
                          <div className="flex items-center justify-between py-1 border-b border-slate-900">
                            <span className="text-slate-400">REST API Gateway:</span>
                            <span className={diagnosticReport.api?.reachable ? "text-emerald-500" : "text-rose-500"}>
                              {diagnosticReport.api?.reachable ? "🟢 Reachable" : diagnosticReport.api?.error?.includes("503") ? "⚠️ paused (503)" : "❌ Unreachable"}
                            </span>
                          </div>
                        </div>
                        ...
                        {diagnosticReport.recommendations?.length > 0 && (
                          <div className="space-y-1.5">
                            <p className="text-[8px] font-black text-rose-400 uppercase tracking-wider">Recommendations:</p>
                            <div className="p-2.5 bg-slate-900 border border-slate-800 rounded-lg text-[10px] font-semibold leading-relaxed text-slate-300 space-y-1 max-h-40 overflow-y-auto custom-scrollbar">
                        ...
                        {diagnosticReport.recommendations?.length === 0 && (
                          <div className="p-2 bg-emerald-950/20 border border-emerald-900/30 rounded-lg text-center text-[10px] text-emerald-400 font-bold">
                            ✓ All database connections are fully operational!
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    <button
                      ...
                      className="w-full py-1.5 bg-slate-800 hover:bg-slate-750 disabled:opacity-50 text-white rounded-lg text-[9px] font-black uppercase tracking-widest transition-all"
                    >
                      {isDiagnosing ? "Running Diagnostic Audit..." : "Retest Connection"}
                    </button>
                  </motion.div>
                )}
              </AnimatePresence>
                  ...
                  >
                    ...
                    <div className="p-2">
                      ...
                      <div className="px-3 py-2 space-y-2">
                        ...
                        <select
                          value={currentUser.summaryEmailFrequency || "NONE"}
                          onChange={(e) => {
                            updateUserProfile(currentUser.id, { 
                              summaryEmailFrequency: e.target.value as "NONE" | "DAILY" | "WEEKLY" 
                            });
                          }}
                          className="w-full bg-slate-50 border border-slate-200 text-slate-700 text-xs rounded-lg px-2 py-1.5 focus:outline-none focus:border-primary/50"
                        >
                          <option value="NONE">Disabled</option>
                          <option value="DAILY">Daily Summary</option>
                          <option value="WEEKLY">Weekly Summary</option>
                        </select>
                        {currentUser.summaryEmailFrequency && currentUser.summaryEmailFrequency !== "NONE" && (
                          <button
                              ...
                              try {
                                ...
                                const res = await fetch("/api/send-summary-email", {
                                  ...
                                  body: JSON.stringify({
                                    to: currentUser.email,
                                    userName: currentUser.name,
                                    frequency: currentUser.summaryEmailFrequency,
                                    pendingCount,
                                    draftsCount,
                                    recentDisbursed
                                  })
                                ...
                                if (res.ok && data.success) {
                                  alert(`Digest sent successfully to ${currentUser.email}! This has been shared via ict.team@pceastandrews.org.`);
                                  updateUserProfile(currentUser.id, {
                                    lastSummaryEmailSentAt: new Date().toISOString()
                                  });
                                } else {
                                  alert("Failed to send: " + (data.error || "Check mailer logs"));
                                }
                              } catch (err: any) {
                            ...
                            className="w-full py-1.5 bg-indigo-50 border border-indigo-200 hover:bg-indigo-100 text-indigo-600 text-[10px] font-bold rounded-lg uppercase tracking-wider transition-all disabled:opacity-50 cursor-pointer"
                          >
                            {sendingTestSummary ? "Dispatching..." : "Send Test Summary"}
                          </button>
                        )}
                      </div>
